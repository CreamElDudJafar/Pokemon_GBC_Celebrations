...
; d = map selected on the Town Map. D survives callfar/Bankswitch.
	...
	call ClearSprites
	call ClearScreen
	; Full-color engine: use the same generic menu palette path as the Pokédex.
	; SET_PAL_GENERIC provides the standard menu/caught-ball colors.
	...
	call CopyVideoDataDouble
	; wcd6d still contains the highlighted Town Map location name here.
	; Draw it once before GetMonName begins reusing wcd6d.
	hlcoord 1, 1
	ld de, wcd6d
	call PlaceString
...
.notCaught
	...
	; Pad wcd6d itself to a fixed 10-character field before printing.
...
; 1F and 2F have a 0 encounter rate in Pokemon Celebrations, so they are omitted.
; The actual wild floors are 3F-7F.
